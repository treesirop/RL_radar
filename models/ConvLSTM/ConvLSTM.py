...
class ConvLSTM(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels_list=None, size: Tuple[int, int] = (280, 360),
                 kernel_size_list=None, forget_bias: float = 0.01):
        super().__init__()
        if hidden_channels_list is None:
            hidden_channels_list = [32, 32]
    ...
    def forward(self, inputs: Tensor, out_len: int = 4) -> Tensor:
...
if __name__ == '__main__':
    ...
    inputs = torch.ones(2, 5, 280, 360).to(device)
    total_params = sum(p.numel() for p in net.parameters())
    print('总参数个数:{}'.format(total_params))
    results = net(inputs, 4)

    print(results.shape) # torch.Size([2, 4, 1, 400, 400])
