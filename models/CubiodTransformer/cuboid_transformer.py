...
@lru_cache()
def compute_cuboid_self_attention_mask(data_shape, cuboid_size, shift_size, strategy, padding_type, device):#与swin相同的操作（窗口滑动后加掩码）
...
class CuboidSelfAttentionLayer(nn.Module):#att
    ...
    def forward(self, x, global_vectors=None):
        ...
        _, num_cuboids, cuboid_volume, _ = reordered_x.shape#1,32,64,256
...
class CuboidTransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.num_blocks > 1:#i-1个downsample
        ...
        self.blocks = nn.ModuleList([nn.Sequential(#按深度、block数目 list[seq(att_block、att_block...),(...),.....]
...
class InitialEncoder(nn.Module):#一层降采样
    def __init__(self,
                 dim,
                 out_dim,
...
class InitialStackPatchMergingEncoder(nn.Module):#多层降采样加patchmerge
...
class CuboidTransformerModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 proj_drop=0.0,
                 ffn_drop=0.0,
                 # inter-attn downsample/upsample att_block之间的降采样参数
                 downsample=2,
                 downsample_type='patch_merge',
                 upsample_type="upsample",
                 upsample_kernel_size=3,
                 # encoder
                 enc_depth=[2,2],#[2*block,2*block]
                 enc_attn_patterns=None,
                 enc_cuboid_size=[(4, 4, 4), (4, 4, 4)],#for a block's att layer
                 enc_cuboid_strategy=[('l', 'l', 'l'), ('d', 'd', 'd')],
                 enc_shift_size=[(0, 0, 0), (0, 0, 0)],
                 enc_use_inter_ffn=True,
                 # decoder
                 dec_depth=[1,1],#[1*block,1*block]
                 dec_cross_start=0,
                 ...
                 # initial_downsample_type == "stack_conv"
                 initial_downsample_stack_conv_num_layers=3,#三层卷积encoder
                 initial_downsample_stack_conv_dim_list=[16, 32, 128],#每层dim
                 initial_downsample_stack_conv_downscale_list=[2, 2, 2],#每层降采系数
                 initial_downsample_stack_conv_num_conv_list=[1,1,1 ],#每层卷积数
        ...
        if self.num_global_vectors > 0:
            self.init_global_vectors = nn.Parameter(
                torch.zeros((self.num_global_vectors, global_dim_ratio*base_units)))#初始化全局向量，类似于memory

        new_input_shape = self.get_initial_encoder_final_decoder(#t-wise 256,256,1->32,32,128
    ...
    def forward(self, x, verbose=False):
        ...
        x = self.enc_pos_embed(x)#b,t,32,32,128
        if self.num_global_vectors > 0:
            init_global_vectors = self.init_global_vectors\
                .expand(B, self.num_global_vectors, self.global_dim_ratio*self.base_units)#b,4,128
...
if __name__ == "__main__":
    device = 'cpu'
    print(device)
    pred_model = CuboidTransformerModel(input_shape=[5,280,360,1],target_shape=[4,280, 360,1]).to(device)
    total = sum(p.numel() for p in pred_model.parameters())
    print(total)
    a = torch.rand(1,5,280,360).to(device)
    #b = torch.rand(1,10,1,256, 256).to(device)
    out_pred = pred_model(a)
    print(out_pred.size()) #b,4,1,400,400