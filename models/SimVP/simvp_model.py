...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gsta',
        ...
        self.final = nn.Conv3d(in_channels=5, out_channels=4, kernel_size=(1, 1, 1))
...
if __name__ == '__main__':
    batch_size = 1
    model = SimVP_Model(in_shape=(5,1,280,360))
    total_params = sum(p.numel() for p in model.parameters())
    print('总参数个数:{}'.format(total_params))

    x = torch.randn(batch_size,5,280,360)
    y = model(x)
    print(y.shape) # torch.Size([1, 4, 1, 400, 400])
